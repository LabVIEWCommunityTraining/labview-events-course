...
. User Events generated after the registration are kept in memory 👉 you should be very careful binding your Registred Events ref to an {es}
...
. Events not consumed by the {es} are deallocated when calling the `Unregister For Events` function 👉 always call this function when you exit an {es}.
